                ...
                if (goodvar_stack_fill_pointer < hd_count_threshold)
                {
                    for (i = 0; i < goodvar_stack_fill_pointer; ++i)
                    {
                        ...
                        if (time_stamp[v] == 0 || step - time_stamp[v] > tabu_step){
                            ...
                            {
                                if (lit_score[v + cur_soln[v] * num_vars] > lit_score[best_var + cur_soln[best_var] * num_vars])
                                    best_var = v;
                                else if (lit_score[v + cur_soln[v] * num_vars] == lit_score[best_var + cur_soln[best_var] * num_vars]){
    ...
    if (hardunsat_stack_fill_pointer > 0)
    {
        if (best_soln_feasible > 0)
            ...
            for (int j = 0; j < clause_lit_count[c]; j++){
                ...
                ucb = lit_score[v + (1 - sense) * num_vars] + lambda * sqrt((log(local_times_hard + 1)) / ((double)(lit_times[v + (1 - sense) * num_vars] + 1)));
                if (ucb > max){
                    max = ucb;
            ...
            lit_times[best_lit]++;
            selected_lits[local_times_hard % backward_step] = best_lit;
...
void BandHS::update_clause_scores2(long long s)
{
    int i, j;
...
void BandHS::update_lit_scores(long long s)
{
    ...
    if (local_times < backward_step)
    {
        for (i = 0; i < local_times; i++)
        {
            ...
            if (abs(lit_score[selected_lits[i]]) > 1000)
                if_exceed = 1;
        }
    }
    else
...
void BandHS::update_lit_scores2(long long s)
{
    int i, j;
    double stemp;
