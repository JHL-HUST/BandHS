...
class ISDist
{
  private:
	...
	double gamma1; // reward discount factor
	double lambda; // exploration bias parameter
	int ArmNum; // number of sampled arms
	int backward_step_hard; // reward delay steps
	double gamma1_hard; // reward discount factor
	double lambda_hard; // exploration bias parameter
    ...
    void update_lit_scores(long long s);
